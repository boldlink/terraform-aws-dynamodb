...
| <a name="input_table_item"></a> [table\_item](#input\_table\_item) | (Required) JSON representation of a map of attribute name/value pairs, one for each attribute. Only the primary key attributes are required; you can optionally provide other attribute name-value pairs for the item. | `string` | n/a | yes |
| <a name="input_table_item_hash_key"></a> [table\_item\_hash\_key](#input\_table\_item\_hash\_key) | (Required) Hash key to use for lookups and identification of the item | `string` | n/a | yes |
